...
/// Process an indexed XCF file into `GenoData` for XPCLR.
///
/// # Examples
///
/// ```ignore
...
/// Process an unindexed XCF file into `GenoData` for XPCLR.
///
/// # Examples
///
/// ```ignore
...
///
/// ```ignore
/// let data = xpclrs::xcf::indexed_xcf("in.bcf".to_string(), &s1, "1", 0, None, (None, None, None, 1)).unwrap();
/// ```
pub fn indexed_xcf(
    xcf_fn: String,
    s1: &[String],
    ...
    let (positions, gt1_data, gd_data): (Vec<_>, Vec<_>, Vec<_>) = if phased {
        reader
            .records()
            .filter_map(|r| {
                let record = r.ok()?;
                ...
                if all_alleles.len() > 2 {
                    ...
                    // If phased, store haplotypes in gt2; else store dosages
                    Some((record.pos() as usize, gt1, gd))
                }
            })
            .multiunzip()
    } else {
...
pub fn readthrough_xcf(
    ...
    log::info!("Skipped {skipped} variants because:");
    log::info!(" - {multiallelic} multiallelic");
    log::info!(" - {monom_gt1} monomorphic/singleton in pop B");
    log::info!(" - {miss_gt1} all-missing in pop A");
    Ok(GenoData {
        positions,
        gt1: gt1_data,
